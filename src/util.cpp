...
const char * const BITCOIN_CONF_FILENAME = "murlicha.conf";
const char * const BITCOIN_PID_FILENAME = "murlichad.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "murlicha" is a composite category enabling all Murlicha-related debug output
            if(ptrCategory->count(string("murlicha"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MurlichaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MurlichaCore
    // Mac: ~/Library/Application Support/MurlichaCore
    // Unix: ~/.murlichacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MurlichaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MurlichaCore";
#else
    // Unix
    return pathRet / ".murlichacore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty murlicha.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override murlicha.conf
